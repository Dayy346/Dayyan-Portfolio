...
## Artifacts
- `tests/qa.spec.ts` now exposes stable `data-testid` attributes (`boot-sequence-dialog`, `start-button`, `desktop`, `window-strip`, etc.), a smarter `skipBootToDesktop` helper (optional login, keyboard enter), and a new login overlay verification test.
